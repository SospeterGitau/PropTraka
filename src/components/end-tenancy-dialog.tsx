...
export function EndTenancyDialog({ isOpen, onClose, onConfirm, tenancy }: EndTenancyDialogProps) {
  ...
  const originalStartDate = new Date(tenancy.startDate);
  const originalEndDate = new Date(tenancy.endDate);
      ...
      <DialogContent aria-describedby="tenancy-description">
        <DialogHeader>
          ...
          <DialogDescription id="tenancy-description">
            Select a new, earlier end date for the tenancy of {tenancy.tenantName}. This will delete any future unpaid rent records.
          </DialogDescription>
        </DialogHeader>
